...
# <a name="Data"></a>
# ### Data Prep
# We now use the `Nemotron` format for conversation style finetunes. We use the [Open Math Reasoning]() dataset which was used to win the [AIMO](https://www.kaggle.com/competitions/ai-mathematical-olympiad-progress-prize-2/leaderboard) (AI Mathematical Olympiad - Progress Prize 2) challenge! We sample 10% of verifiable reasoning traces that used DeepSeek R1, and whicht got > 95% accuracy. Nemotron renders multi turn conversations like below:
# 
# ```
# <|im_start|>user
# Hello!<|im_end|>
...
model.save_pretrained("nemotron_nano_lora")  # Local saving
tokenizer.save_pretrained("nemotron_nano_lora")
# model.push_to_hub("your_name/nemotron_nano_lora", token = "YOUR_HF_TOKEN") # Online saving
# tokenizer.push_to_hub("your_name/nemotron_nano_lora", token = "YOUR_HF_TOKEN") # Online saving
...
if False:
    from unsloth import FastLanguageModel
    model, tokenizer = FastLanguageModel.from_pretrained(
        model_name = "nemotron_nano_lora", # YOUR MODEL YOU USED FOR TRAINING
        max_seq_length = 2048,
        load_in_4bit = True,
    )
...
# Merge to 16bit
if False:
    model.save_pretrained_merged("nemotron_nano_finetune_16bit", tokenizer, save_method = "merged_16bit",)
if False: # Pushing to HF Hub
    model.push_to_hub_merged("HF_USERNAME/nemotron_nano_finetune_16bit", tokenizer, save_method = "merged_16bit", token = "YOUR_HF_TOKEN")

# Merge to 4bit
if False:
    model.save_pretrained_merged("nemotron_nano_finetune_4bit", tokenizer, save_method = "merged_4bit",)
if False: # Pushing to HF Hub
    model.push_to_hub_merged("HF_USERNAME/nemotron_nano_finetune_4bit", tokenizer, save_method = "merged_4bit", token = "YOUR_HF_TOKEN")

# Just LoRA adapters
if False:
    model.save_pretrained("nemotron_nano_lora")
    tokenizer.save_pretrained("nemotron_nano_lora")
if False: # Pushing to HF Hub
    model.push_to_hub("HF_USERNAME/nemotron_nano_lora", token = "YOUR_HF_TOKEN")
    tokenizer.push_to_hub("HF_USERNAME/nemotron_nano_lora", token = "YOUR_HF_TOKEN")
...
# Save to 8bit Q8_0
if False:
    model.save_pretrained_gguf("nemotron_nano_finetune", tokenizer,)
# Remember to go to https://huggingface.co/settings/tokens for a token!
# And change hf to your username!
if False:
    model.push_to_hub_gguf("HF_USERNAME/nemotron_nano_finetune", tokenizer, token = "YOUR_HF_TOKEN")

# Save to 16bit GGUF
if False:
    model.save_pretrained_gguf("nemotron_nano_finetune", tokenizer, quantization_method = "f16")
if False: # Pushing to HF Hub
    model.push_to_hub_gguf("HF_USERNAME/nemotron_nano_finetune", tokenizer, quantization_method = "f16", token = "YOUR_HF_TOKEN")

# Save to q4_k_m GGUF
if False:
    model.save_pretrained_gguf("nemotron_nano_finetune", tokenizer, quantization_method = "q4_k_m")
if False: # Pushing to HF Hub
    model.push_to_hub_gguf("HF_USERNAME/nemotron_nano_finetune", tokenizer, quantization_method = "q4_k_m", token = "YOUR_HF_TOKEN")

# Save to multiple GGUF options - much faster if you want multiple!
if False:
    model.push_to_hub_gguf(
        "HF_USERNAME/nemotron_nano_finetune", # Change hf to your username!
        tokenizer,
...
# Now, use the `nemotron_nano_finetune.Q8_0.gguf` file or `nemotron_nano_finetune.Q4_K_M.gguf` file in llama.cpp.
# 
