...
# <a name="Data"></a>
# ### Data Prep
# We now use the `GLM Flash` format for conversation style finetunes. We use the [Open Math Reasoning]() dataset which was used to win the [AIMO](https://www.kaggle.com/competitions/ai-mathematical-olympiad-progress-prize-2/leaderboard) (AI Mathematical Olympiad - Progress Prize 2) challenge! We sample 10% of verifiable reasoning traces that used DeepSeek R1, and whicht got > 95% accuracy. GLM 4.7 renders multi turn conversations like below:
# 
# ```
# [gMASK]<sop><|system|>
...
if False: # Pushing to HF Hub
    model.push_to_hub_merged("HF_USERNAME/glm_flash_finetune_16bit", tokenizer, save_method = "merged_16bit", token = "")

# Merge to 4bit
if False:
    model.save_pretrained_merged("glm_flash_finetune_4bit", tokenizer, save_method = "merged_4bit",)
if False: # Pushing to HF Hub
    model.push_to_hub_merged("HF_USERNAME/glm_flash_finetune_4bit", tokenizer, save_method = "merged_4bit", token = "")
...
if False:
    ...
    tokenizer.save_pretrained("glm_flash_lora")
if False: # Pushing to HF Hub
    model.push_to_hub("HF_USERNAME/glm_flash_lora", token = "")
    tokenizer.push_to_hub("HF_USERNAME/glm_flash_lora", token = "")
...
if False:
    model.push_to_hub_gguf("HF_USERNAME/glm_flash_finetune", tokenizer, token = "")

# Save to 16bit GGUF
if False:
    model.save_pretrained_gguf("glm_flash_finetune", tokenizer, quantization_method = "f16")
if False: # Pushing to HF Hub
    model.push_to_hub_gguf("HF_USERNAME/glm_flash_finetune", tokenizer, quantization_method = "f16", token = "")

# Save to q4_k_m GGUF
if False:
    model.save_pretrained_gguf("glm_flash_finetune", tokenizer, quantization_method = "q4_k_m")
if False: # Pushing to HF Hub
    model.push_to_hub_gguf("HF_USERNAME/glm_flash_finetune", tokenizer, quantization_method = "q4_k_m", token = "")
