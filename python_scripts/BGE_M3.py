...
if False: # Pushing to HF Hub
    model.push_to_hub_merged("HF_USERNAME/bge_m3_finetune_16bit", tokenizer = model.tokenizer, save_method = "merged_16bit", token = "")

# Just LoRA adapters
if False:
    model.save_pretrained("bge_m3_lora")
if False: # Pushing to HF Hub
    model.push_to_hub("HF_USERNAME/bge_m3_lora", token = "")
...
if False:
    model.push_to_hub_gguf("HF_USERNAME/bge_m3_finetune", token = "")

# Save to 16bit GGUF
if False:
    model.save_pretrained_gguf("bge_m3_finetune", quantization_method = "f16")
if False: # Pushing to HF Hub
    model.push_to_hub_gguf("HF_USERNAME/bge_m3_finetune", quantization_method = "f16", token = "")

# Save to q4_k_m GGUF
if False:
    model.save_pretrained_gguf("bge_m3_finetune", quantization_method = "q4_k_m")
if False: # Pushing to HF Hub
    model.push_to_hub_gguf("HF_USERNAME/bge_m3_finetune", quantization_method = "q4_k_m", token = "")
