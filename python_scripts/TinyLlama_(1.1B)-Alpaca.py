...
model.save_pretrained("tinyllama_lora")  # Local saving
tokenizer.save_pretrained("tinyllama_lora")
# model.push_to_hub("your_name/tinyllama_lora", token = "YOUR_HF_TOKEN") # Online saving
# tokenizer.push_to_hub("your_name/tinyllama_lora", token = "YOUR_HF_TOKEN") # Online saving
...
if False:
    from unsloth import FastLanguageModel
    model, tokenizer = FastLanguageModel.from_pretrained(
        model_name = "tinyllama_lora", # YOUR MODEL YOU USED FOR TRAINING
        max_seq_length = max_seq_length,
        dtype = dtype,
        load_in_4bit = load_in_4bit,
    )
...
if False:
    ...
    model = AutoPeftModelForCausalLM.from_pretrained(
        "tinyllama_lora", # YOUR MODEL YOU USED FOR TRAINING
        load_in_4bit = load_in_4bit,
    )
    tokenizer = AutoTokenizer.from_pretrained("tinyllama_lora")
...
if False: model.push_to_hub_merged("HF_USERNAME/tinyllama_finetune_16bit", tokenizer, save_method = "merged_16bit", token = "")

# Merge to 4bit
if False: model.save_pretrained_merged("tinyllama_finetune_4bit", tokenizer, save_method = "merged_4bit",)
if False: model.push_to_hub_merged("HF_USERNAME/tinyllama_finetune_4bit", tokenizer, save_method = "merged_4bit", token = "")
...
if False:
    ...
    tokenizer.save_pretrained("tinyllama_lora")
if False:
    model.push_to_hub("HF_USERNAME/tinyllama_lora", token = "")
    tokenizer.push_to_hub("HF_USERNAME/tinyllama_lora", token = "")
...
if False: model.push_to_hub_gguf("HF_USERNAME/tinyllama_finetune", tokenizer, token = "")

# Save to 16bit GGUF
if False: model.save_pretrained_gguf("tinyllama_finetune", tokenizer, quantization_method = "f16")
if False: model.push_to_hub_gguf("HF_USERNAME/tinyllama_finetune", tokenizer, quantization_method = "f16", token = "")

# Save to q4_k_m GGUF
if False: model.save_pretrained_gguf("tinyllama_finetune", tokenizer, quantization_method = "q4_k_m")
if False: model.push_to_hub_gguf("HF_USERNAME/tinyllama_finetune", tokenizer, quantization_method = "q4_k_m", token = "")
