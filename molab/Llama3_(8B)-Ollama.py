...
def _(dataset):
    ...
    )  # Select more to handle longer conversations
    return (dataset_1,)
...
def _(dataset_1):
    ...
    dataset_2 = standardize_sharegpt(dataset_1)
    return (dataset_2,)
...
@app.cell
def _(dataset_2, tokenizer):
    chat_template = "Below are some instructions that describe some tasks. Write responses that appropriately complete each request.\n\n### Instruction:\n{INPUT}\n\n### Response:\n{OUTPUT}"
    from unsloth import apply_chat_template

    dataset_3 = apply_chat_template(
        dataset_2, tokenizer=tokenizer, chat_template=chat_template
...
@app.cell
def _(dataset_3, max_seq_length, model_1, tokenizer):
...
@app.cell
def _(FastLanguageModel, TextStreamer, model_1, tokenizer):
    FastLanguageModel.for_inference(model_1)  # Enable native 2x faster inference
    messages_1 = [
        {
            "role": "user",
        ...
        {
            "role": "assistant",
            "content": "The fibonacci sequence continues as 13, 21, 34, 55 and 89.",
        },
        {"role": "user", "content": "What is France's tallest tower called?"},
    ]  # Change below!
    input_ids_1 = tokenizer.apply_chat_template(
        messages_1, add_generation_prompt=True, return_tensors="pt"
    ).to("cuda")
...
def _(TextStreamer, dtype, load_in_4bit, max_seq_length, model_1, tokenizer):
    ...
    messages_2 = [
        {
            "role": "user",
            "content": "Describe anything special about a sequence. Your input is 1, 1, 2, 3, 5, 8,",
        }
    ]
    input_ids_2 = tokenizer.apply_chat_template(
        messages_2, add_generation_prompt=True, return_tensors="pt"
    ).to("cuda")  # Change below!
...
def _(mo):
    mo.md(r"""
    We use `subprocess` to start `Ollama` up in a non blocking fashion! In your own desktop, you can simply open up a new `terminal` and type `ollama serve`, but in molab, we have to use this hack!
    """)
    return
...
def _():
    ...
    # !curl http://localhost:11434/api/chat -d '{ "model": "unsloth_model", "messages": [ { "role": "user", "content": "Continue the Fibonacci sequence: 1, 1, 2, 3, 5, 8," } ] }'
    return
