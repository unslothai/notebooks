...
@app.cell
def _(model_1, tokenizer, train_dataset_1):
    from vllm import SamplingParams

    sampling_params = SamplingParams(temperature=1.0, top_k=50, max_tokens=1024)
    prompt = train_dataset_1[100]["prompt"]
    if isinstance(prompt, list):
        prompt = tokenizer.apply_chat_template(
            prompt, tokenize=False, add_generation_prompt=True
        )
    outputs = model_1.fast_generate(
        {
            "prompt": prompt,
            "multi_modal_data": {"image": train_dataset_1[100]["image"]},
        },
        sampling_params,
    )
...
@app.cell
def _(SamplingParams, model_1, tokenizer, train_dataset_1):
    sampling_params_1 = SamplingParams(temperature=1.0, top_k=50, max_tokens=1024)
    prompt_1 = train_dataset_1[165]["prompt"]
    if isinstance(prompt_1, list):
        prompt_1 = tokenizer.apply_chat_template(
            prompt_1, tokenize=False, add_generation_prompt=True
        )
    outputs_1 = model_1.fast_generate(
        {
            "prompt": prompt_1,
            "multi_modal_data": {"image": train_dataset_1[165]["image"]},
        },
        sampling_params_1,
        lora_request=model_1.load_lora("grpo_lora"),
    )
    print(outputs_1[0].outputs[0].text)
    return
