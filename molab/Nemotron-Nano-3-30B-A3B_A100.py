...
def _():
    ...
    model, tokenizer = FastLanguageModel.from_pretrained(
        model_name="unsloth/Nemotron-3-Nano-30B-A3B",  # YOUR MODEL YOU USED FOR TRAINING
...
@app.cell
def _(model_1, tokenizer):
    model_1.save_pretrained("nemotron_nano_lora")
...
def _():
    if False:
        ...
        _model, _tokenizer = _FastLanguageModel.from_pretrained(
            model_name="nemotron_nano_lora", max_seq_length=2048, load_in_4bit=True  # YOUR MODEL YOU USED FOR TRAINING
        )
    return
...
def _(model_1, tokenizer):
    ...
    if False:
        model_1.save_pretrained_merged(
            "nemotron_nano_finetune_16bit", tokenizer, save_method="merged_16bit"
        )
    if False:  # Pushing to HF Hub
        model_1.push_to_hub_merged(
            "HF_USERNAME/nemotron_nano_finetune_16bit",
            tokenizer,
    ...
    if False:
        # Merge to 4bit
        model_1.save_pretrained_merged(
            "nemotron_nano_finetune_4bit", tokenizer, save_method="merged_4bit"
        )
    if False:
        model_1.push_to_hub_merged(
            "HF_USERNAME/nemotron_nano_finetune_4bit",
            tokenizer,
    ...
    if False:
        ...
        tokenizer.save_pretrained("nemotron_nano_lora")
    if False:
        model_1.push_to_hub("HF_USERNAME/nemotron_nano_lora", token="YOUR_HF_TOKEN")
        tokenizer.push_to_hub(
            "HF_USERNAME/nemotron_nano_lora", token="YOUR_HF_TOKEN"
        )  # Pushing to HF Hub
    return
...
def _(model_1, tokenizer):
    # Save to 8bit Q8_0
    if False:
        model_1.save_pretrained_gguf("nemotron_nano_finetune", tokenizer)
    if False:
        model_1.push_to_hub_gguf(
            "HF_USERNAME/nemotron_nano_finetune", tokenizer, token="YOUR_HF_TOKEN"
        )
    if False:
        # Save to 16bit GGUF
        model_1.save_pretrained_gguf(
            "nemotron_nano_finetune", tokenizer, quantization_method="f16"
        )
    if False:
        model_1.push_to_hub_gguf(
            "HF_USERNAME/nemotron_nano_finetune",
            tokenizer,
    ...
    if False:
        model_1.save_pretrained_gguf(
            "nemotron_nano_finetune", tokenizer, quantization_method="q4_k_m"
        )
    # Save to q4_k_m GGUF
    if False:
        model_1.push_to_hub_gguf(
