...
def _(tokenizer):
    ...
    tokenizer_1 = get_chat_template(tokenizer, chat_template="gemma3")
    return (tokenizer_1,)
...
def _(dataset):
    ...
    dataset_1 = dataset.map(convert_to_chatml)
    return (dataset_1,)
...
def _(dataset_2, model_1, tokenizer_1):
    ...
    trainer = SFTTrainer(
        ...
        args=SFTConfig(
            ...
            warmup_steps=5,
            max_steps=100,
            learning_rate=5e-05,  # Reduce to 2e-5 for long training runs
            logging_steps=1,
            optim="adamw_8bit",
            weight_decay=0.001,
...
def _(dataset_2, model_1, tokenizer_1):
    messages = [
        {"role": "system", "content": dataset_2["conversations"][10][0]["content"]},
        {"role": "user", "content": dataset_2["conversations"][10][1]["content"]},
    ]
    ...
    _ = model_1.generate(
        **tokenizer_1(text, return_tensors="pt").to("cuda"),
        max_new_tokens=125,
        temperature=1,
        top_p=0.95,
        top_k=64,
        streamer=TextStreamer(tokenizer_1, skip_prompt=True),
...
@app.cell
def _(model_1, tokenizer_1):
    model_1.save_pretrained("gemma_3_lora")
    tokenizer_1.save_pretrained("gemma_3_lora")
    return
...
def _():
    if False:
        ...
        _model, _tokenizer = _FastLanguageModel.from_pretrained(
            model_name="gemma_3_lora", max_seq_length=2048, load_in_4bit=False  # YOUR MODEL YOU USED FOR TRAINING
        )
    return
...
def _(model_1, tokenizer_1):
    ...
    if False:
        model_1.save_pretrained_merged(
            "gemma_3_finetune_16bit", tokenizer_1, save_method="merged_16bit"
        )
    if False:  # Pushing to HF Hub
        model_1.push_to_hub_merged(
            "HF_USERNAME/gemma_3_finetune_16bit",
            tokenizer_1,
    ...
    if False:
        # Merge to 4bit
        model_1.save_pretrained_merged(
            "gemma_3_finetune_4bit", tokenizer_1, save_method="merged_4bit"
        )
    if False:
        model_1.push_to_hub_merged(
            "HF_USERNAME/gemma_3_finetune_4bit",
            tokenizer_1,
    ...
    if False:
        ...
        tokenizer_1.save_pretrained("gemma_3_lora")
    if False:
        model_1.push_to_hub("HF_USERNAME/gemma_3_lora", token="YOUR_HF_TOKEN")
        tokenizer_1.push_to_hub(
            "HF_USERNAME/gemma_3_lora", token="YOUR_HF_TOKEN"
        )  # Pushing to HF Hub
    return
...
def _(model_1, tokenizer_1):
    if False:  # Change to True to save to GGUF
        model_1.save_pretrained_gguf(
            "gemma_3_finetune", tokenizer_1, quantization_method="Q8_0"
