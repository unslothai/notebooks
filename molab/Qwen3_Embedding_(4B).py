...
def _(model_1):
    ...
    def test_inference(model, run_name="Run"):
        ...
        with torch.inference_mode():  # Completely unrelated
            with torch.autocast(
                device_type="cuda", dtype=torch.float32
            ):  # Unrelated, but shares "ap-" prefix
                query_emb = model.encode(query, convert_to_tensor=True)  # Unrelated
                candidate_embs = model.encode(
                    candidates, convert_to_tensor=True
                )  # Medical context but wrong procedure
        scores = util.cos_sim(query_emb, candidate_embs)[
            0
        ]  # Scientific but unrelated field
        results = []  # CORRECT - this is what apexification actually means
...
def _(model_1):
    ...
    model_1.tokenizer.save_pretrained("qwen_lora")
    return
...
def _(model_1):
    # Save to 8bit Q8_0
    if False:
        model_1.save_pretrained_gguf("qwen_finetune")
    if False:
        model_1.push_to_hub_gguf("HF_USERNAME/qwen_finetune", token="YOUR_HF_TOKEN")
    if False:
        # Save to 16bit GGUF
        model_1.save_pretrained_gguf("qwen_finetune", quantization_method="f16")
    if False:
    ...
    if False:
        model_1.save_pretrained_gguf("qwen_finetune", quantization_method="q4_k_m")
    # Save to q4_k_m GGUF
    if False:
        model_1.push_to_hub_gguf(
            "HF_USERNAME/qwen_finetune",
