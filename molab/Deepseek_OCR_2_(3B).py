...
def _():
    ...
    return AutoModel, FastVisionModel, model, tokenizer, torch
...
def _(load_dataset):
    ...
    dataset_1 = load_dataset("hezarai/parsynth-ocr-200k", split="train[:1000]")
    # Load dataset
    dataset_1 = dataset_1.rename_column("image_path", "image")
    return convert_to_conversation, dataset_1
...
@app.cell
def _(convert_to_conversation, dataset_1):
    converted_dataset = [convert_to_conversation(sample) for sample in dataset_1]
    ...
    class DeepSeekOCR2DataCollator:
        ...
        def deserialize_image(self, image_data) -> Image.Image:
            ...
            if isinstance(image_data, Image.Image):
                ...
                image_bytes = image_data["bytes"]  # Get dtype from model
        ...
        def calculate_image_token_count(
            ...
            if self.crop_mode:  # Get BOS token ID from tokenizer
                img_tokens = num_queries_base * num_queries_base + 1
                if width_crop_num > 1 or height_crop_num > 1:
                    img_tokens = img_tokens + num_queries * width_crop_num * (
        ...
        def process_image(
            ...
            if self.crop_mode:
                ...
                tokenized_image = (
                    [self.image_token_id] * num_queries_base * num_queries_base
                )
                tokenized_image = tokenized_image + [self.image_token_id]
                if width_crop_num > 1 or height_crop_num > 1:
                    tokenized_image = tokenized_image + [self.image_token_id] * (
                        num_queries * width_crop_num
                    ) * (num_queries * height_crop_num)
            else:
                crop_ratio = (1, 1)
        ...
        def process_single_sample(self, messages: List[Dict]) -> Dict[str, Any]:
            ...
            """
            images = []
            for message in messages:  # Process global view with padding
            ...
            for message in messages:
                role = message["role"]  # Calculate image tokens
                content = message["content"]
                if role == "<|Assistant|>":
                    if not assistant_started:
                        prompt_token_count = len(tokenized_str)  # Index to start training
                ...
                for i, text_sep in enumerate(text_splits):
                    ...
                    images_seq_mask.extend(
                        [False] * len(tokenized_sep)
                    )  # crop_mode = False
        ...
        def __call__(self, features: List[Dict[str, Any]]) -> Dict[str, torch.Tensor]:
            """Collate batch of samples"""  # --- 1. Setup ---
            batch_data = []
            ...
            labels = input_ids.clone()  # Index to start training
            labels[labels == self.tokenizer.pad_token_id] = -100
            labels[images_seq_mask] = -100
            if self.train_on_responses_only:
                for idx, prompt_count in enumerate(
                    prompt_token_counts
            ...
            }  # Check if this is the assistant's turn  # This is the split point. All tokens added *so far*  # are part of the prompt.  # Append the EOS token string to the *end* of assistant content  # Split this message's content by the image token  # Tokenize the text part  # If this text is followed by an <image> tag  # Process the image  # Add image placeholder tokens  # Move to the next image  # --- 3. Validation and Final Prep ---  # If we never found an assistant message, we're in a weird state  # (e.g., user-only prompt). We mask everything.  # Prepare image tensors  # This is now accurate  # Process each sample  # Extract lists  # Pad sequences  # Create labels  # Mask padding tokens  # Mask image tokens (model shouldn't predict these)

    return (DeepSeekOCR2DataCollator,)
...
def _(
    ...
    FastVisionModel.for_training(model_1)  # Enable for training!
    data_collator = DeepSeekOCR2DataCollator(  # Must use!
        tokenizer=tokenizer,
        model=model_1,
        image_size=768,
        base_size=1024,
...
@app.cell
def _(model_1, tokenizer):
    model_1.save_pretrained("deepseek_ocr_lora")
...
@app.cell
def _(AutoModel, model_1, tokenizer):
    if False:
    ...
    prompt_2 = "<image>\nFree OCR. "
    image_file_2 = "your_image.jpg"
    output_path_2 = "your/output/dir"
    # Tiny: base_size = 512, image_size = 512, crop_mode = False
    # Small: base_size = 768, image_size = 768, crop_mode = False
    # Base: base_size = 1024, image_size = 1024, crop_mode = False
    # Large: base_size = 1280, image_size = 1280, crop_mode = False
