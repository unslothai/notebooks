...
def _():
    ...
    model, tokenizer = FastLanguageModel.from_pretrained(
        model_name="unsloth/Mistral-Small-Instruct-2409",  # YOUR MODEL YOU USED FOR TRAINING
...
def _(model_1, tokenizer):
    model_1.save_pretrained("mistral_small_lora")
...
def _(load_in_4bit):
    if False:
        ...
        _model = _AutoPeftModelForCausalLM.from_pretrained(
            "mistral_small_lora", load_in_4bit=load_in_4bit
        )
        _tokenizer = _AutoTokenizer.from_pretrained("mistral_small_lora")
    return
...
def _(model_1, tokenizer):
    ...
    if False:
        model_1.save_pretrained_merged(
            "mistral_small_finetune_16bit", tokenizer, save_method="merged_16bit"
        )
    if False:
        # Merge to 4bit
        model_1.push_to_hub_merged(
            "HF_USERNAME/mistral_small_finetune_16bit",
            tokenizer,
    ...
    if False:
        model_1.save_pretrained_merged(
            "mistral_small_finetune_4bit", tokenizer, save_method="merged_4bit"
        )
    # Just LoRA adapters
    if False:
        model_1.push_to_hub_merged(
            "HF_USERNAME/mistral_small_finetune_4bit",
            tokenizer,
    ...
    if False:
        ...
        tokenizer.save_pretrained("mistral_small_lora")
    if False:
        model_1.push_to_hub("HF_USERNAME/mistral_small_lora", token="YOUR_HF_TOKEN")
...
def _(model_1, tokenizer):
    ...
    if False:
        ...
        model_1.save_pretrained_gguf("mistral_small_finetune", tokenizer)
    if False:
        # Save to 16bit GGUF
        model_1.push_to_hub_gguf(
            "HF_USERNAME/mistral_small_finetune", tokenizer, token="YOUR_HF_TOKEN"
        )
    if False:
        model_1.save_pretrained_gguf(
            "mistral_small_finetune", tokenizer, quantization_method="f16"
        )
    # Save to q4_k_m GGUF
    if False:
        model_1.push_to_hub_gguf(
            "HF_USERNAME/mistral_small_finetune",
            tokenizer,
    ...
    if False:
        ...
        model_1.save_pretrained_gguf(
            "mistral_small_finetune", tokenizer, quantization_method="q4_k_m"
        )
    if False:
        model_1.push_to_hub_gguf(
            "HF_USERNAME/mistral_small_finetune",
