...
def _(subprocess):
    ...
    subprocess.call(["synthetic-data-kit", "system-check"])
    return
...
def _(filenames, subprocess):
    qa_pairs_filenames = [
        f"data/generated/arxiv_org_{i}_qa_pairs.json" for i in range(len(filenames[:3]))
    ]
    for filename_1 in qa_pairs_filenames:
        subprocess.call(
            [
                "synthetic-data-kit",
                "-c",
...
def _(filenames):
    ...
    final_filenames = [
        f"data/final/arxiv_org_{i}_qa_pairs_ft.json" for i in range(len(filenames[:3]))
    ]
...
def _(dataset, tokenizer):
    ...
    dataset_1 = dataset.map(formatting_prompts_func, batched=True)
    return (dataset_1,)
...
def _(model_1, tokenizer):
    ...
    _ = model_1.generate(
        input_ids=inputs, streamer=text_streamer, max_new_tokens=256, temperature=0.1
...
def _(TextStreamer, model_1, tokenizer):
    messages_1 = [{"role": "user", "content": "What are some benefits of the BLT?"}]
    ...
    _ = model_1.generate(
        ...
        max_new_tokens=256,
        temperature=0.1,  # Higher temp makes more diverse datasets
    )  # Must add for generation
    return
...
@app.cell
def _(model_1, tokenizer):
    model_1.save_pretrained("meta_synthetic_data_lora")
...
def _(TextStreamer, dtype, load_in_4bit, max_seq_length, model_1, tokenizer):
    if False:
        ...
        _model, _tokenizer = _FastLanguageModel.from_pretrained(
            ...
            max_seq_length=max_seq_length,  # Longer sequence lengths will be slower!
            dtype=dtype,
...
def _(model_1, tokenizer):
    ...
    if False:
        model_1.save_pretrained_merged(
            "meta_synthetic_data_finetune_16bit", tokenizer, save_method="merged_16bit"
        )
    ...
    if False:
        # Merge to 4bit
        model_1.save_pretrained_merged(
            "meta_synthetic_data_finetune_4bit", tokenizer, save_method="merged_4bit"
        )
    if False:
        model_1.push_to_hub_merged(
...
def _(model_1, tokenizer):
    # Save to 8bit Q8_0
    if False:
        model_1.save_pretrained_gguf("meta_synthetic_data_finetune", tokenizer)
    if False:  # Change to True to upload finetune
        model_1.push_to_hub_gguf(
            "HF_USERNAME/meta_synthetic_data_finetune", tokenizer, token="YOUR_HF_TOKEN"
        )
    if False:
        # Save to 16bit GGUF
        model_1.save_pretrained_gguf(
            "meta_synthetic_data_finetune", tokenizer, quantization_method="f16"
        )
    if False:
        model_1.push_to_hub_gguf(
    ...
    if False:
        model_1.save_pretrained_gguf(
            "meta_synthetic_data_finetune", tokenizer, quantization_method="q4_k_m"
        )
    # Save to q4_k_m GGUF
    if False:
        model_1.push_to_hub_gguf(
