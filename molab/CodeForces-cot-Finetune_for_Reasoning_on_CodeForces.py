...
def _():
    ...
    model, tokenizer = FastLanguageModel.from_pretrained(
        model_name="unsloth/Qwen2.5-Coder-7B-Instruct-bnb-4bit",  # YOUR MODEL YOU USED FOR TRAINING
...
@app.cell
def _(ds, tokenizer):
    tokenizer.apply_chat_template(ds["train"]["messages"][0], tokenize=False)
    return
...
@app.cell
def _(ds, max_seq_length, model_1, tokenizer):
...
@app.cell
def _(FastLanguageModel, model_1, tokenizer):
...
def _(model_1, tokenizer):
    model_1.save_pretrained("codeforces_cot_finetune_lora")
...
def _(load_in_4bit):
    if False:
        ...
        _model = _AutoPeftModelForCausalLM.from_pretrained(
            "codeforces_cot_finetune_lora", load_in_4bit=load_in_4bit
        )
        _tokenizer = _AutoTokenizer.from_pretrained("codeforces_cot_finetune_lora")
    return
...
def _(model_1, tokenizer):
    ...
    if False:
        model_1.save_pretrained_merged(
            "codeforces_cot_finetune_16bit", tokenizer, save_method="merged_16bit"
        )
    if False:
        # Merge to 4bit
        model_1.push_to_hub_merged(
            "HF_USERNAME/codeforces_cot_finetune_16bit",
            tokenizer,
    ...
    if False:
        model_1.save_pretrained_merged(
            "codeforces_cot_finetune_4bit", tokenizer, save_method="merged_4bit"
        )
    # Just LoRA adapters
    if False:
        model_1.push_to_hub_merged(
            "HF_USERNAME/codeforces_cot_finetune_4bit",
            tokenizer,
    ...
    if False:
        model_1.push_to_hub(
            "HF_USERNAME/codeforces_cot_finetune_lora", token="YOUR_HF_TOKEN"
        )
...
def _(model_1, tokenizer):
    ...
    if False:
        ...
        model_1.save_pretrained_gguf("codeforces_cot_finetune", tokenizer)
    if False:
        # Save to 16bit GGUF
        model_1.push_to_hub_gguf(
            "HF_USERNAME/codeforces_cot_finetune", tokenizer, token="YOUR_HF_TOKEN"
        )
    if False:
        model_1.save_pretrained_gguf(
            "codeforces_cot_finetune", tokenizer, quantization_method="f16"
        )
    # Save to q4_k_m GGUF
    if False:
        model_1.push_to_hub_gguf(
            "HF_USERNAME/codeforces_cot_finetune",
            tokenizer,
    ...
    if False:
        ...
        model_1.save_pretrained_gguf(
            "codeforces_cot_finetune", tokenizer, quantization_method="q4_k_m"
        )
    if False:
        model_1.push_to_hub_gguf(
            "HF_USERNAME/codeforces_cot_finetune",
