...
def _(dataset, torch):
    ...
    dataset_1 = dataset.map(formatting_audio_func, remove_columns=["audio"])
    print("Moving Bicodec model and Wav2Vec2Model to cpu.")
    audio_tokenizer.model.cpu()
    audio_tokenizer.feature_extractor.cpu()
    torch.cuda.empty_cache()  # Squeeze batch dim
    return audio_tokenizer, dataset_1, np
...
def _(
    ...
    def generate_speech_from_text(
        ...
        """
        Generates speech audio from text using default voice control parameters.  # Enable native 2x faster inference

        Args:
            text (str): The text input to be converted to speech.
            temperature (float): Sampling temperature for generation.
            top_k (int): Top-k sampling parameter.  # Generation temperature
            top_p (float): Top-p (nucleus) sampling parameter.  # Generation top_k
            max_new_audio_tokens (int): Max number of new tokens to generate (limits audio length).  # Generation top_p
            device (torch.device): Device to run inference on.  # Max tokens for audio part
...
def _(model_1, tokenizer):
    model_1.save_pretrained("spark_tts_lora")
...
def _(model_1, tokenizer):
    ...
    if False:
        model_1.save_pretrained_merged(
            "spark_tts_finetune_16bit", tokenizer, save_method="merged_16bit"
        )
    if False:
        # Merge to 4bit
        model_1.push_to_hub_merged(
            "HF_USERNAME/spark_tts_finetune_16bit",
            tokenizer,
    ...
    if False:
        model_1.save_pretrained_merged(
            "spark_tts_finetune_4bit", tokenizer, save_method="merged_4bit"
        )
    # Just LoRA adapters
    if False:
        model_1.push_to_hub_merged(
            "HF_USERNAME/spark_tts_finetune_4bit",
            tokenizer,
    ...
    if False:
        ...
        tokenizer.save_pretrained("spark_tts_lora")
    if False:
        model_1.push_to_hub("HF_USERNAME/spark_tts_lora", token="YOUR_HF_TOKEN")
