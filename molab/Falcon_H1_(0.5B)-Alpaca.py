...
def _(model_1, tokenizer):
    model_1.save_pretrained("falcon_h1_lora")
...
def _(
    ...
    if False:
        ...
        _model, _tokenizer = _FastLanguageModel.from_pretrained(
            model_name="falcon_h1_lora",  # Choose any model from https://huggingface.co/collections/tiiuae/falcon-h1-6819f2795bc406da60fab8df
            max_seq_length=max_seq_length,  # Choose any! We auto support RoPE Scaling internally!
            dtype=dtype,
            load_in_4bit=load_in_4bit,  # Use 4bit quantization to reduce memory usage. Can be False.
        )
...
def _(load_in_4bit):
    if False:
        ...
        _model = _AutoPeftModelForCausalLM.from_pretrained(
            "falcon_h1_lora", load_in_4bit=load_in_4bit
        )
        _tokenizer = _AutoTokenizer.from_pretrained("falcon_h1_lora")
    return
...
def _(model_1, tokenizer):
    ...
    if False:
        model_1.save_pretrained_merged(
            "falcon_h1_finetune_16bit", tokenizer, save_method="merged_16bit"
        )
    if False:
        # Merge to 4bit
        model_1.push_to_hub_merged(
            "HF_USERNAME/falcon_h1_finetune_16bit",
            tokenizer,
    ...
    if False:
        model_1.save_pretrained_merged(
            "falcon_h1_finetune_4bit", tokenizer, save_method="merged_4bit"
        )
    # Just LoRA adapters
    if False:
        model_1.push_to_hub_merged(
            "HF_USERNAME/falcon_h1_finetune_4bit",
            tokenizer,
    ...
    if False:
        ...
        tokenizer.save_pretrained("falcon_h1_lora")
    if False:
        model_1.push_to_hub("HF_USERNAME/falcon_h1_lora", token="YOUR_HF_TOKEN")
...
def _(model_1, tokenizer):
    ...
    if False:
        ...
        model_1.save_pretrained_gguf("falcon_h1_finetune", tokenizer)
    if False:
        # Save to 16bit GGUF
        model_1.push_to_hub_gguf(
            "HF_USERNAME/falcon_h1_finetune", tokenizer, token="YOUR_HF_TOKEN"
        )
    if False:
        model_1.save_pretrained_gguf(
            "falcon_h1_finetune", tokenizer, quantization_method="f16"
        )
    # Save to q4_k_m GGUF
    if False:
        model_1.push_to_hub_gguf(
            "HF_USERNAME/falcon_h1_finetune",
            tokenizer,
    ...
    if False:
        ...
        model_1.save_pretrained_gguf(
            "falcon_h1_finetune", tokenizer, quantization_method="q4_k_m"
        )
    if False:
        model_1.push_to_hub_gguf(
            "HF_USERNAME/falcon_h1_finetune",
