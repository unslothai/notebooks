...
def _(requests, time):
    ...
    print(
        f"Waiting for resources server at {config.host}:{config.port}",
        end="",
        flush=True,
    )
    for _i in range(90):
...
def _(os, tokenizer):
    ...
    if not os.path.exists(dataset_path):
        raise FileNotFoundError(
            f"Dataset not found at {dataset_path}. Run the setup cell above first."
        )
    train_data = []
    max_length_seen = 0
    ...
    train_dataset = Dataset.from_list(train_data)  # extract prompt from nemo gym format
...
@app.cell
def _(model_name, requests, verify_endpoint):
    import numpy as np
...
def _(model, tokenizer):
    ...
    if False:
        model.save_pretrained_merged(
            "nemo_gym_sudoku_finetune_4bit", tokenizer, save_method="mxfp4"
        )
    if False:
        model.push_to_hub_merged(
            "repo_id/nemo_gym_sudoku_finetune_4bit",
            tokenizer,
            token="YOUR_HF_TOKEN",
            save_method="mxfp4",
        )
    ...
    if False:  # Pushing to HF Hub
        model.push_to_hub_merged(
            "HF_USERNAME/nemo_gym_sudoku_finetune_16bit",
            tokenizer,
