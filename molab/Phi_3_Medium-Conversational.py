...
def _(model_1):
    ...
    model_1.save_pretrained("phi_lora")  # Local saving
    return
...
def _(TextStreamer, dtype, load_in_4bit, max_seq_length, model_1, tokenizer_2):
    if False:
        ...
        _model, _tokenizer = _FastLanguageModel.from_pretrained(
            model_name="phi_lora",  # Choose ANY! eg teknium/OpenHermes-2.5-Mistral-7B
            max_seq_length=max_seq_length,  # Choose any! We auto support RoPE Scaling internally!
            dtype=dtype,  # None for auto detection. Float16 for Tesla T4, V100, Bfloat16 for Ampere+
            load_in_4bit=load_in_4bit,  # Use 4bit quantization to reduce memory usage. Can be False.
        )
...
def _(load_in_4bit):
    if False:
        ...
        _model = _AutoPeftModelForCausalLM.from_pretrained(
            "phi_lora", load_in_4bit=load_in_4bit
        )
        _tokenizer = _AutoTokenizer.from_pretrained("phi_lora")
    return
...
def _(model_1, tokenizer_2):
    ...
    if False:
        model_1.save_pretrained_merged(
            "phi_finetune_16bit", tokenizer_2, save_method="merged_16bit"
        )
    if False:
        # Merge to 4bit
        model_1.push_to_hub_merged(
            "HF_USERNAME/phi_finetune_16bit",
            tokenizer_2,
    ...
    if False:
        model_1.save_pretrained_merged(
            "phi_finetune_4bit", tokenizer_2, save_method="merged_4bit"
        )
    # Just LoRA adapters
    if False:
        model_1.push_to_hub_merged(
            "HF_USERNAME/phi_finetune_4bit",
            tokenizer_2,
            save_method="merged_4bit",
            token="YOUR_HF_TOKEN",
        )
    if False:
        model_1.save_pretrained("phi_lora")
        tokenizer_2.save_pretrained("phi_lora")
    if False:
        model_1.push_to_hub("HF_USERNAME/phi_lora", token="YOUR_HF_TOKEN")
        tokenizer_2.push_to_hub("HF_USERNAME/phi_lora", token="YOUR_HF_TOKEN")
    return
...
def _(model_1, tokenizer_2):
    # Save to 8bit Q8_0
    if False:
        model_1.save_pretrained_gguf("phi_finetune", tokenizer_2)
    if False:
        # Save to 16bit GGUF
        model_1.push_to_hub_gguf(
            "HF_USERNAME/phi_finetune", tokenizer_2, token="YOUR_HF_TOKEN"
        )
    if False:
        model_1.save_pretrained_gguf(
            "phi_finetune", tokenizer_2, quantization_method="f16"
        )
    # Save to q4_k_m GGUF
    if False:
        model_1.push_to_hub_gguf(
            "HF_USERNAME/phi_finetune",
            tokenizer_2,
    ...
    if False:
        model_1.save_pretrained_gguf(
            "phi_finetune", tokenizer_2, quantization_method="q4_k_m"
        )
    if False:
        model_1.push_to_hub_gguf(
            "HF_USERNAME/phi_finetune",
