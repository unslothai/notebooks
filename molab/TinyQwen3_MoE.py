...
def _(lora_rank, max_seq_length, model_name):
    ...
    model = FastLanguageModel.get_peft_model(
        ...
        target_modules=[
            ...
            "up_proj",
            "down_proj",
            "gate_up_proj",  # Enable LoRA on MoE layers
        ],
...
def _(model, tokenizer):
    ...
    if False:
        model.save_pretrained_merged(
            "tinyqwen_finetune_16bit",
            tokenizer,
            save_method="merged_16bit",
        )
    if False:
        model.push_to_hub_merged(
            "HF_USERNAME/tinyqwen_finetune_16bit",
            tokenizer,
            save_method="merged_16bit",
            token="YOUR_HF_TOKEN",
        )
    ...
    if False:
        model.save_pretrained_merged(
            "tinyqwen_finetune_4bit",
            tokenizer,
            save_method="merged_4bit",
        )
    if False:
        model.push_to_hub_merged(
            "HF_USERNAME/tinyqwen_finetune_4bit",
            tokenizer,
            save_method="merged_4bit",
            token="YOUR_HF_TOKEN",
        )
...
def _(model, tokenizer):
    ...
    if False:
        model.push_to_hub_gguf(
            "HF_USERNAME/tinyqwen_finetune", tokenizer, token="YOUR_HF_TOKEN"
        )

    # Save to 16bit GGUF
    if False:
        model.save_pretrained_gguf(
            "tinyqwen_finetune", tokenizer, quantization_method="f16"
        )
    if False:
        model.push_to_hub_gguf(
            "HF_USERNAME/tinyqwen_finetune",
            tokenizer,
    ...
    if False:
        model.save_pretrained_gguf(
            "tinyqwen_finetune", tokenizer, quantization_method="q4_k_m"
        )
    if False:
        model.push_to_hub_gguf(
            "HF_USERNAME/tinyqwen_finetune",
