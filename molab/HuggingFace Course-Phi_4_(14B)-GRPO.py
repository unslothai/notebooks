...
def _(model, tokenizer):
    ...
    if False:
        model.push_to_hub_gguf(
            "HF_USERNAME/phi_finetune", tokenizer, token="YOUR_HF_TOKEN"
        )

    # Save to 16bit GGUF
    if False:
        model.save_pretrained_gguf("phi_finetune", tokenizer, quantization_method="f16")
    if False:
        model.push_to_hub_gguf(
    ...
    if False:
        model.save_pretrained_gguf(
            "phi_finetune", tokenizer, quantization_method="q4_k_m"
        )
    if False:
        model.push_to_hub_gguf(
            "HF_USERNAME/phi_finetune",
