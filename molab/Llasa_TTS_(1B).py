...
def _():
    ...
    return FastLanguageModel, max_seq_length, model, tokenizer, torch
...
@app.cell
def _(OUTPUT_DIR, dataset, max_seq_length, tokenizer, torch):
    ...
    def preprocess_and_save(
        ...
        for idx, example in tqdm(
            enumerate(dataset), total=num_samples, desc=f"Processing"
        ):
            try:
                ...
                if (
                    "audio" not in example
                    or "array" not in example["audio"]
                    or "sampling_rate" not in example["audio"]
                    or (example["audio"]["array"] is None)
                ):
                    skipped_count = skipped_count + 1
                    continue
                ...
                if original_sr != sample_rate:
                    waveform = torchaudio.functional.resample(
                        waveform, original_sr, sample_rate
                    )
    ...
    class MemmapTTSDataset(Dataset):
        def __init__(
            self, data_path: str, tokenizer: AutoTokenizer, max_length: int = 2048
        ):
            ...
            if self.tokenizer.pad_token_id is None:
                if self.tokenizer.eos_token_id is not None:
                    ...
                    raise ValueError(
                        "Tokenizer passed to Dataset must have pad_token_id set."
                    )
            ...
            if not self.shape or len(self.shape) != 2 or self.shape[0] == 0:
                ...
                self.memmap_data = np.memmap(
                    memmap_file, dtype="int32", mode="r", shape=self.shape
                )
                self.length = self.shape[0]
            try:
                self.speech_generation_start_id = tokenizer.convert_tokens_to_ids(
                    "<|SPEECH_GENERATION_START|>"
                )
                self.speech_generation_end_id = tokenizer.convert_tokens_to_ids(
                    "<|SPEECH_GENERATION_END|>"
                )
                self.text_understanding_start_id = tokenizer.convert_tokens_to_ids(
                    "<|TEXT_UNDERSTANDING_START|>"
                )
                self.text_understanding_end_id = tokenizer.convert_tokens_to_ids(
                    "<|TEXT_UNDERSTANDING_END|>"
                )
                assert isinstance(self.pad_token_id, int)
            except Exception as token_err:
                raise ValueError(
                    f"Tokenizer is missing required special tokens or pad_token_id. Error: {token_err}"
                )
        ...
        def pad_sequence_torch(self, sequence, max_length, value=0):
            ...
            if current_len >= max_length:
                ...
                padding = torch.full(
                    (padding_size,), value, dtype=sequence.dtype, device=sequence.device
                )
        ...
        def __getitem__(self, idx):
            ...
            try:
                text_start_idx = (
                    (input_ids_tensor_raw == self.text_understanding_start_id)
                    .nonzero(as_tuple=True)[0][0]
                    .item()
                )
                text_end_idx = (
                    (input_ids_tensor_raw == self.text_understanding_end_id)
                    .nonzero(as_tuple=True)[0][0]
                    .item()
                )
                speech_start_idx = (
                    (input_ids_tensor_raw == self.speech_generation_start_id)
                    .nonzero(as_tuple=True)[0][0]
                    .item()
                )
                speech_end_marker_indices = (
                    input_ids_tensor_raw == self.speech_generation_end_id
                ).nonzero(as_tuple=True)[0]
                pad_start_indices = (input_ids_tensor_raw == self.pad_token_id).nonzero(
                    as_tuple=True
                )[0]
                ...
                if (
                    ...
                    < speech_start_idx
                    < speech_end_idx
                    < len(input_ids_tensor_raw)
                ):
                    raise ValueError("Parsed indices are invalid or out of order")
                original_text_sequence = input_ids_tensor_raw[:speech_start_idx]
                original_speech_sequence_with_markers = input_ids_tensor_raw[
                    speech_start_idx : speech_end_idx + 1
                ]
                chat = [
                    {
                        "role": "user",
                        "content": f"Convert the text to speech:<|TEXT_UNDERSTANDING_START|>",
                    },
                ...
                try:
                    ...
                    if "add_generation_prompt" in sig.parameters:
                        templated_ids_list = self.tokenizer.apply_chat_template(
                    ...
                    final_ids_list = self.replace_tagged_token(
                        templated_ids_list,
                        self.text_understanding_start_id,
                        original_text_sequence.tolist(),
                    )
                    final_ids_list = self.replace_tagged_token(
                        final_ids_list,
                        self.speech_generation_start_id,
                        original_speech_sequence_with_markers.tolist(),
                    )
                    input_ids = torch.tensor(final_ids_list, dtype=torch.long)
                    try:
                        speech_gen_idx_in_final = (
            ...
            if speech_gen_idx_in_final != -1 and speech_gen_idx_in_final < len(
                input_ids
            ):
                labels[speech_gen_idx_in_final:] = input_ids[
                    speech_gen_idx_in_final:
                ].clone()
            attention_mask = (input_ids != self.pad_token_id).long()
            labels[input_ids == self.pad_token_id] = self.ignore_index
            input_ids = self.pad_sequence_torch(
                input_ids, self.max_length, value=self.pad_token_id
            )
            attention_mask = self.pad_sequence_torch(
                attention_mask, self.max_length, value=0
            )
            labels = self.pad_sequence_torch(
                labels, self.max_length, value=self.ignore_index
            )
            return {
                "input_ids": input_ids,
                "labels": labels,
    ...
    preprocess_and_save(
        ...
        max_length=max_seq_length,  # We trained our model with a max length of 2048
        device=DEVICE,
    )
    try:
        train_dataset = MemmapTTSDataset(
            data_path=OUTPUT_DIR, tokenizer=tokenizer, max_length=max_seq_length
        )
        print(
            f"Dataset loaded for split 'train'. Number of samples: {len(train_dataset)}"
        )
    except Exception as e:
    ...
    return codec_model, train_dataset
...
@app.cell
def _(FastLanguageModel, codec_model, input_text, model_1, tokenizer, torch):
    # @title Run Inference
    ...
    display(
        Audio(gen_wav[0, 0, :].cpu().numpy(), rate=16000)
    )
    return
...
def _(model_1, tokenizer):
    model_1.save_pretrained("llasa_tts_lora")
...
def _(model_1, tokenizer):
    ...
    if False:
        model_1.save_pretrained_merged(
            "llasa_tts_finetune_16bit", tokenizer, save_method="merged_16bit"
        )
    if False:
        # Merge to 4bit
        model_1.push_to_hub_merged(
            "HF_USERNAME/llasa_tts_finetune_16bit",
            tokenizer,
    ...
    if False:
        model_1.save_pretrained_merged(
            "llasa_tts_finetune_4bit", tokenizer, save_method="merged_4bit"
        )
    # Just LoRA adapters
    if False:
        model_1.push_to_hub_merged(
            "HF_USERNAME/llasa_tts_finetune_4bit",
            tokenizer,
    ...
    if False:
        ...
        tokenizer.save_pretrained("llasa_tts_lora")
    if False:
        model_1.push_to_hub("HF_USERNAME/llasa_tts_lora", token="YOUR_HF_TOKEN")
