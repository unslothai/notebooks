...
def _(torch):
    ...
    def preprocess_example(example):
        ...
        try:
            ...
            print(
                f"Error processing example with text '{example['text'][:50]}...': {e}"
            )
            return None
        required_keys = [
            "input_ids",
    ...
    return Audio, processed_ds, processor_1, raw_ds
...
@app.cell
def _(Audio, model_1, processor_1, torch):
...
def _(model_1, processor_1):
    model_1.save_pretrained("sesame_csm_lora")
    processor_1.save_pretrained("sesame_csm_lora")
    return
...
def _(model_1, processor_1):
    ...
    if False:
        model_1.save_pretrained_merged(
            "sesame_csm_finetune_16bit", processor_1, save_method="merged_16bit"
        )
    if False:
        # Merge to 4bit
        model_1.push_to_hub_merged(
            "HF_USERNAME/sesame_csm_finetune_16bit",
            processor_1,
    ...
    if False:
        model_1.save_pretrained_merged(
            "sesame_csm_finetune_4bit", processor_1, save_method="merged_4bit"
        )
    # Just LoRA adapters
    if False:
        model_1.push_to_hub_merged(
            "HF_USERNAME/sesame_csm_finetune_4bit",
            processor_1,
    ...
    if False:
        ...
        processor_1.save_pretrained("sesame_csm_lora")
    if False:
        model_1.push_to_hub("HF_USERNAME/sesame_csm_lora", token="YOUR_HF_TOKEN")
        processor_1.push_to_hub("HF_USERNAME/sesame_csm_lora", token="YOUR_HF_TOKEN")
    return
