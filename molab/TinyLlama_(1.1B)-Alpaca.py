...
def _(model_1, tokenizer):
    model_1.save_pretrained("tinyllama_lora")
...
def _(
    ...
    if False:
        ...
        _model, _tokenizer = _FastLanguageModel.from_pretrained(
            model_name="tinyllama_lora",  # "unsloth/tinyllama" for 16bit loading
            max_seq_length=max_seq_length,  # Choose any! We auto support RoPE Scaling internally!
            dtype=dtype,  # None for auto detection. Float16 for Tesla T4, V100, Bfloat16 for Ampere+
            load_in_4bit=load_in_4bit,  # Use 4bit quantization to reduce memory usage. Can be False.
        )
    ...
    _text_streamer = TextStreamer(tokenizer)
    _ = model_1.generate(**_inputs, streamer=_text_streamer, max_new_tokens=64)
    return
...
def _(load_in_4bit):
    if False:
        ...
        _model = _AutoPeftModelForCausalLM.from_pretrained(
            "tinyllama_lora", load_in_4bit=load_in_4bit
        )
        _tokenizer = _AutoTokenizer.from_pretrained("tinyllama_lora")
    return
...
def _(model_1, tokenizer):
    ...
    if False:
        model_1.save_pretrained_merged(
            "tinyllama_finetune_16bit", tokenizer, save_method="merged_16bit"
        )
    if False:
        # Merge to 4bit
        model_1.push_to_hub_merged(
            "HF_USERNAME/tinyllama_finetune_16bit",
            tokenizer,
    ...
    if False:
        model_1.save_pretrained_merged(
            "tinyllama_finetune_4bit", tokenizer, save_method="merged_4bit"
        )
    # Just LoRA adapters
    if False:
        model_1.push_to_hub_merged(
            "HF_USERNAME/tinyllama_finetune_4bit",
            tokenizer,
    ...
    if False:
        ...
        tokenizer.save_pretrained("tinyllama_lora")
    if False:
        model_1.push_to_hub("HF_USERNAME/tinyllama_lora", token="YOUR_HF_TOKEN")
...
def _(model_1, tokenizer):
    # Save to 8bit Q8_0
    if False:
        model_1.save_pretrained_gguf("tinyllama_finetune", tokenizer)
    if False:
        # Save to 16bit GGUF
        model_1.push_to_hub_gguf(
            "HF_USERNAME/tinyllama_finetune", tokenizer, token="YOUR_HF_TOKEN"
        )
    if False:
        model_1.save_pretrained_gguf(
            "tinyllama_finetune", tokenizer, quantization_method="f16"
        )
    # Save to q4_k_m GGUF
    if False:
        model_1.push_to_hub_gguf(
            "HF_USERNAME/tinyllama_finetune",
            tokenizer,
    ...
    if False:
        model_1.save_pretrained_gguf(
            "tinyllama_finetune", tokenizer, quantization_method="q4_k_m"
        )
    if False:
        model_1.push_to_hub_gguf(
            "HF_USERNAME/tinyllama_finetune",
