...
def _():
    ...
    dataset = load_dataset(
        "sentence-transformers/all-nli", "pair", split="train[:100000]"
    )
    return (dataset,)
...
def _(model_1):
    ...
    def test_inference(model, run_name="Run"):
        ...
        candidates = [
            "An athlete is practicing a sport outdoors.",
            "The player is about to score a goal for his team.",
            "A person is sitting quietly on the grass.",
        ...
        scores = util.cos_sim(query_emb, candidate_embs)[0]  # Correct - entailment
        results = []  # Neutral - possible but uncertain
        for i, score in enumerate(scores):  # Wrong - contradicts action
            results.append(
                (candidates[i], score.item())
            )  # Wrong - unrelated definition
...
@app.cell
def _(model_1):
    model_1.save_pretrained("all_minilm_l6_v2_lora")
    model_1.tokenizer.save_pretrained("all_minilm_l6_v2_lora")
    return
...
def _(model_1):
    ...
    if False:
        model_1.save_pretrained_merged(
            "all_minilm_l6_v2_finetune_16bit",
            tokenizer=model_1.tokenizer,
            save_method="merged_16bit",
        )
    if False:  # Pushing to HF Hub
        model_1.push_to_hub_merged(
            "HF_USERNAME/all_minilm_l6_v2_finetune_16bit",
            tokenizer=model_1.tokenizer,
            save_method="merged_16bit",
    ...
    if False:
        model_1.push_to_hub(
            "HF_USERNAME/all_minilm_l6_v2_lora", token="YOUR_HF_TOKEN"
        )  # Pushing to HF Hub
    return
...
def _(model_1):
    # Save to 8bit Q8_0
    if False:
        model_1.save_pretrained_gguf("all_minilm_l6_v2_finetune")
    if False:
        model_1.push_to_hub_gguf(
            "HF_USERNAME/all_minilm_l6_v2_finetune", token="YOUR_HF_TOKEN"
        )
    if False:
        # Save to 16bit GGUF
        model_1.save_pretrained_gguf(
            "all_minilm_l6_v2_finetune", quantization_method="f16"
        )
    if False:
        model_1.push_to_hub_gguf(
            "HF_USERNAME/all_minilm_l6_v2_finetune",
            quantization_method="f16",
            token="YOUR_HF_TOKEN",  # Get a token at https://huggingface.co/settings/tokens
        )  # Pushing to HF Hub
    if False:
        model_1.save_pretrained_gguf(
            "all_minilm_l6_v2_finetune", quantization_method="q4_k_m"
        )
    # Save to q4_k_m GGUF
    if False:
        model_1.push_to_hub_gguf(
