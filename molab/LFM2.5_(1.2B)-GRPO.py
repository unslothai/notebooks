...
def _(model, tokenizer):
    ...
    _ = model.generate(
        **inputs, max_new_tokens=4096, use_cache=True, streamer=TextStreamer(tokenizer)
    )
    return (TextStreamer,)
...
@app.cell
def _(reasoning_end, reasoning_start, system_prompt, tokenizer):
    tokenizer.apply_chat_template(
        [
            ...
            {
                "role": "assistant",
                "content": f"{reasoning_start}I think it's 2.{reasoning_end}2",
            },
...
def _(tokenizer):
    ...
    solution_end_regex = (
        r"</think>[\s]*" + "(?:" + re.escape(tokenizer.eos_token) + ")?"
    )
...
def _(match_format):
    match_format.findall(
        "<think>Let me think!</think>\boxed\{2\}",
    )
    return
...
def _(reasoning_end, reasoning_start):
    def match_format_approximately(completions, **kwargs):
        ...
        for completion in completions:
            ...
            scores.append(score)
        return scores  # No need to reward the opening tag since we always prepend it!
...
def _(TextStreamer, model, tokenizer):
    messages_1 = [{"role": "user", "content": "What is the sqrt of 101?"}]
    inputs_1 = tokenizer.apply_chat_template(
        messages_1, add_generation_prompt=True, return_tensors="pt", return_dict=True
    ).to(model.device)
    _ = model.generate(
        **inputs_1,
        temperature=0.1,
...
@app.cell
def _(TextStreamer, model, system_prompt, tokenizer):
    messages_2 = [
        {"role": "system", "content": system_prompt},
        {"role": "user", "content": "What is the sqrt of 101?"},
    ]
    inputs_2 = tokenizer.apply_chat_template(
        messages_2, add_generation_prompt=True, return_tensors="pt", return_dict=True
    ).to(model.device)
    _ = model.generate(
        **inputs_2,
        temperature=0.1,
...
def _(model, tokenizer):
    ...
    # Save to 16bit GGUF
    if False:
        model.save_pretrained_gguf("lfm_finetune", tokenizer, quantization_method="f16")
    if False:
        model.push_to_hub_gguf(
