...
def _():
    ...
    return max_seq_length, model, model_name, tokenizer
...
def _(mo):
    ...
    molab is auto-detected and the `uv_pip_set_python=true` flag is added when needed.
    """)
    return
...
def _():
    ...
    # Refresh uv before asking it for that interpreter. molab ships a uv whose
    ...
    def _venv_python_satisfies_floor():
        ...
        if probe.returncode != 0:
            return False
        floor = tuple(
            int(p) for p in _GYM_PYTHON.lstrip("><=!~ ").split(".") if p.isdigit()
        )
    ...
    if _sync.returncode != 0 and not _venv_python_satisfies_floor():
        print(
            "Rebuilding the NeMo Gym venv: its Python is below the floor NeMo Gym requires."
        )
        subprocess.run(
    ...
    try:
        ...
        for _ in range(120):
            ...
            raise RuntimeError("NeMo Gym servers did not start within 6 minutes.")
        print("\nHead server ready!")
    return os, requests, time
...
def _(requests, time):
    ...
    try:
        ...
        raise RuntimeError(
            "Could not connect to NeMo Gym head server on port 11000. Make sure the setup cell above ran successfully."
        )
    ...
    if _missing:
        raise RuntimeError(
            f"Head server is missing required server configs: {_missing}. Please stop any existing NeMo Gym server and re-run the setup cell above."
        )
...
def _(os, tokenizer):
    ...
    train_data = []
    examples_by_server = {}
    max_length_seen = 0  # (dataset path, server name)
    ...
    for dataset_path, server_name in dataset_configs:
        ...
        if not os.path.exists(expanded_path):
            raise FileNotFoundError(
                f"Dataset not found at {expanded_path}. Run the setup cell above first."
            )
...
@app.cell
def _(get_verify_endpoint, model_name, requests):
    import numpy as np
    ...
    def reward_fn(completions, prompts=None, **kwargs):
        ...
        for i, completion in enumerate(completions):
            ...
            try:
                ...
                reward = (
                    resp.json().get("reward", 0.0) if resp.status_code == 200 else 0.0
                )
...
def _(
    ...
    max_prompt_length = (  # +1 just in case as in other unsloth examples
        max_length_seen + 1
    )  # +1 just in case as in other unsloth examples
    ...
    training_args = GRPOConfig(
        ...
        num_generations=8,
        max_prompt_length=max_prompt_length,  # +1 just in case as in other unsloth examples
...
def _(model, tokenizer):
    ...
    if False:
        model.save_pretrained_merged(
            "nemo_gym_multi_finetune_4bit", tokenizer, save_method="mxfp4"
        )
    if False:
        model.push_to_hub_merged(
            "repo_id/nemo_gym_multi_finetune_4bit",
            tokenizer,
            token="YOUR_HF_TOKEN",
            save_method="mxfp4",
        )
    ...
    if False:  # Pushing to HF Hub
        model.push_to_hub_merged(
            "HF_USERNAME/nemo_gym_multi_finetune_16bit",
            tokenizer,
