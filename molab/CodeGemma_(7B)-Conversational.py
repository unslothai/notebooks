...
def _():
    ...
    model, tokenizer = FastLanguageModel.from_pretrained(
        ...
        max_seq_length=max_seq_length,  # Gemma sadly only supports max 8192 for now
        dtype=dtype,
...
@app.cell
def _(dataset, model_1, tokenizer_1):
...
def _(FastLanguageModel, get_chat_template, model_1, tokenizer_1):
    ...
    outputs = model_1.generate(input_ids=_inputs, max_new_tokens=64, use_cache=True)
...
def _(FastLanguageModel, model_1, tokenizer_2):
    ...
    _text_streamer = TextStreamer(tokenizer_2)
    _ = model_1.generate(
        input_ids=_inputs, streamer=_text_streamer, max_new_tokens=128, use_cache=True
    )
    return (TextStreamer,)
...
def _(model_1):
    ...
    model_1.save_pretrained("codegemma_lora")  # Local saving
    return
...
def _(TextStreamer, dtype, load_in_4bit, max_seq_length, model_1, tokenizer_2):
    if False:
        ...
        _model, _tokenizer = _FastLanguageModel.from_pretrained(
            model_name="codegemma_lora",  # Choose ANY! eg teknium/OpenHermes-2.5-Mistral-7B
            max_seq_length=max_seq_length,  # Gemma sadly only supports max 8192 for now
            dtype=dtype,
            load_in_4bit=load_in_4bit,  # Use 4bit quantization to reduce memory usage. Can be False.
        )
        _FastLanguageModel.for_inference(_model)
    _messages = [{"from": "human", "value": "What is a famous tall tower in Paris?"}]
    _inputs = tokenizer_2.apply_chat_template(
        _messages, tokenize=True, add_generation_prompt=True, return_tensors="pt"
    ).to("cuda")
    _text_streamer = TextStreamer(tokenizer_2)
...
def _(load_in_4bit):
    if False:
        ...
        _model = _AutoPeftModelForCausalLM.from_pretrained(
            "codegemma_lora", load_in_4bit=load_in_4bit
        )
        _tokenizer = _AutoTokenizer.from_pretrained("codegemma_lora")
    return
...
def _(model_1, tokenizer_2):
    ...
    if False:
        model_1.save_pretrained_merged(
            "codegemma_finetune_16bit", tokenizer_2, save_method="merged_16bit"
        )
    if False:
        # Merge to 4bit
        model_1.push_to_hub_merged(
            "HF_USERNAME/codegemma_finetune_16bit",
            tokenizer_2,
    ...
    if False:
        model_1.save_pretrained_merged(
            "codegemma_finetune_4bit", tokenizer_2, save_method="merged_4bit"
        )
    # Just LoRA adapters
    if False:
        model_1.push_to_hub_merged(
            "HF_USERNAME/codegemma_finetune_4bit",
            tokenizer_2,
            save_method="merged_4bit",
            token="YOUR_HF_TOKEN",
        )
    if False:
        model_1.save_pretrained("codegemma_lora")
        tokenizer_2.save_pretrained("codegemma_lora")
    if False:
        model_1.push_to_hub("HF_USERNAME/codegemma_lora", token="YOUR_HF_TOKEN")
        tokenizer_2.push_to_hub("HF_USERNAME/codegemma_lora", token="YOUR_HF_TOKEN")
    return
...
def _(model_1, tokenizer_2):
    # Save to 8bit Q8_0
    if False:
        model_1.save_pretrained_gguf("codegemma_finetune", tokenizer_2)
    if False:
        # Save to 16bit GGUF
        model_1.push_to_hub_gguf(
            "HF_USERNAME/codegemma_finetune", tokenizer_2, token="YOUR_HF_TOKEN"
        )
    if False:
        model_1.save_pretrained_gguf(
            "codegemma_finetune", tokenizer_2, quantization_method="f16"
        )
    # Save to q4_k_m GGUF
    if False:
        model_1.push_to_hub_gguf(
            "HF_USERNAME/codegemma_finetune",
            tokenizer_2,
    ...
    if False:
        model_1.save_pretrained_gguf(
            "codegemma_finetune", tokenizer_2, quantization_method="q4_k_m"
        )
    if False:
        model_1.push_to_hub_gguf(
            "HF_USERNAME/codegemma_finetune",
