...
@app.cell
def _(collate_fn, dataset_1, model_1, processor):
...
def _(model_1, processor):
    if True:  # Change to True to save finetune!
        model_1.save_pretrained_merged("gemma-3n", processor)
    return
...
def _(model_1, processor):
    if False:  # Change to True to upload finetune
        model_1.push_to_hub_merged(
            "HF_ACCOUNT/gemma-3N-finetune", processor, token="YOUR_HF_TOKEN"
        )
    return
...
def _(model_1, processor):
    if False:  # Change to True to save to GGUF
        model_1.save_pretrained_gguf(
            "gemma_3n_finetune", processor, quantization_method="Q8_0"
