...
@app.cell
def _(SamplingParams, model, text_1):
    sampling_params_1 = SamplingParams(temperature=1.0, top_k=50, max_tokens=256)
    output_1 = (
        model.fast_generate(
            [text_1], sampling_params=sampling_params_1, lora_request=None
        )[0]
        .outputs[0]
        .text
    )
...
def _(SamplingParams, model):
    ...
    sampling_params_2 = SamplingParams(temperature=0.3, top_k=50, max_tokens=1024)
    output_2 = (
        model.fast_generate(
            [text_2], sampling_params=sampling_params_2, lora_request=None
        )[0]
        .outputs[0]
        .text
    )
...
def _(SamplingParams, model, system_prompt, tokenizer):
    ...
    sampling_params_3 = SamplingParams(temperature=0.3, top_k=50, max_tokens=2048)
    output_3 = (
        model.fast_generate(
            text_3,
