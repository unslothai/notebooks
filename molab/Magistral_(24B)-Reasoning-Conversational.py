...
def _(mo):
    mo.md(r"""
    To run this notebook on your L4 molab Pro instance, hit the **▶ Run all** button in the bottom-right corner - or use `Ctrl/Cmd + Shift + R`.
...
def _():
    ...
    model, tokenizer = FastLanguageModel.from_pretrained(
        model_name="unsloth/Magistral-Small-2509-unsloth-bnb-4bit",  # YOUR MODEL YOU USED FOR TRAINING
...
def _(reasoning_dataset, tokenizer):
    reasoning_dataset_1 = reasoning_dataset.map(generate_conversation)
    reasoning_conversations = [
        tokenizer.apply_chat_template(conv["conversations"], tokenize=False)
        for conv in reasoning_dataset_1
    ]
...
@app.cell
def _(combined_dataset, model_1, tokenizer):
...
@app.cell
def _(model_1, tokenizer):
    model_1.save_pretrained("magistral_lora")
...
def _():
    if False:
        ...
        _model, _tokenizer = _FastLanguageModel.from_pretrained(
            model_name="magistral_lora", max_seq_length=2048, load_in_4bit=True  # YOUR MODEL YOU USED FOR TRAINING
        )
    return
...
def _(model_1, tokenizer):
    ...
    if False:
        model_1.save_pretrained_merged(
            "magistral_finetune_16bit", tokenizer, save_method="merged_16bit"
        )
    if False:  # Pushing to HF Hub
        model_1.push_to_hub_merged(
            "HF_USERNAME/magistral_finetune_16bit",
            tokenizer,
    ...
    if False:
        # Merge to 4bit
        model_1.save_pretrained_merged(
            "magistral_finetune_4bit", tokenizer, save_method="merged_4bit"
        )
    if False:
        model_1.push_to_hub_merged(
            "HF_USERNAME/magistral_finetune_4bit",
            tokenizer,
    ...
    if False:
        ...
        tokenizer.save_pretrained("magistral_lora")
    if False:
        model_1.push_to_hub("HF_USERNAME/magistral_lora", token="YOUR_HF_TOKEN")
        tokenizer.push_to_hub(
            "HF_USERNAME/magistral_lora", token="YOUR_HF_TOKEN"
        )  # Pushing to HF Hub
    return
...
def _(model_1, tokenizer):
    # Save to 8bit Q8_0
    if False:
        model_1.save_pretrained_gguf("magistral_finetune", tokenizer)
    if False:
        model_1.push_to_hub_gguf(
            "HF_USERNAME/magistral_finetune", tokenizer, token="YOUR_HF_TOKEN"
        )
    if False:
        # Save to 16bit GGUF
        model_1.save_pretrained_gguf(
            "magistral_finetune", tokenizer, quantization_method="f16"
        )
    if False:
        model_1.push_to_hub_gguf(
            "HF_USERNAME/magistral_finetune",
            tokenizer,
    ...
    if False:
        model_1.save_pretrained_gguf(
            "magistral_finetune", tokenizer, quantization_method="q4_k_m"
        )
    # Save to q4_k_m GGUF
    if False:
        model_1.push_to_hub_gguf(
