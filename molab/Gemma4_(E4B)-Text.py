...
def _(mo):
    mo.md(r"""
    To run this notebook on a molab L4 instance, hit the **▶ Run all** button in the bottom-right corner - or use `Ctrl/Cmd + Shift + R`.
...
def _():
    ...
    model, tokenizer = FastModel.from_pretrained(
        model_name="unsloth/gemma-4-E4B-it",  # YOUR MODEL YOU USED FOR TRAINING
...
def _(do_gemma_4_inference):
    ...
    do_gemma_4_inference(messages_1)
    return
...
def _(do_gemma_4_inference):
    ...
    do_gemma_4_inference(messages_2, max_new_tokens=256)
    return (audio_file,)
...
@app.cell
def _(audio_file, do_gemma_4_inference, sloth_link):
    messages_3 = [
        {
            "role": "user",
            "content": [
    ...
    do_gemma_4_inference(messages_3, max_new_tokens=256)
    return
...
def _(tokenizer):
    ...
    tokenizer_1 = get_chat_template(tokenizer, chat_template="gemma-4")
...
def _(get_chat_template, model_1, tokenizer_1):
    tokenizer_2 = get_chat_template(tokenizer_1, chat_template="gemma-4")
    messages_4 = [
        {
            "role": "user",
            "content": [
...
def _(model_1, tokenizer_2):
    ...
    tokenizer_2.save_pretrained("gemma_4_lora")
    return
...
def _(model_1, tokenizer_2):
    if False:  # Change to True to save finetune!
        model_1.save_pretrained_merged("gemma-4-finetune", tokenizer_2)
    return
...
def _(model_1, tokenizer_2):
    if False:  # Change to True to upload finetune
        model_1.push_to_hub_merged(
            "HF_ACCOUNT/gemma-4-finetune", tokenizer_2, token="YOUR_HF_TOKEN"
        )
    return
...
def _(model_1, tokenizer_2):
    if False:  # Change to True to save to GGUF
        model_1.save_pretrained_gguf(
            "gemma_4_finetune", tokenizer_2, quantization_method="Q8_0"
