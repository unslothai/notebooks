...
def _(dataset, tokenizer, torch):
    ...
    def add_codes(example):
        ...
        example["codes_list"] = codes_list  # Always initialize codes_list to None
        return example

    dataset_1 = dataset.map(add_codes, remove_columns=["audio"])
    tokeniser_length = 128256
    start_of_text = 128000
    end_of_text = 128009  # If there's a valid audio array, tokenise it
    start_of_speech = tokeniser_length + 1
    end_of_speech = tokeniser_length + 2
    start_of_human = tokeniser_length + 3
    end_of_human = tokeniser_length + 4
    start_of_ai = tokeniser_length + 5
    end_of_ai = tokeniser_length + 6  # Keep codes_list as None if we fail
    pad_token = tokeniser_length + 7
    audio_tokens_start = tokeniser_length + 10
    dataset_1 = dataset_1.filter(lambda x: x["codes_list"] is not None)
    dataset_1 = dataset_1.filter(lambda x: len(x["codes_list"]) > 0)
    ...
    dataset_1 = dataset_1.map(remove_duplicate_frames)
    ...
    dataset_1 = dataset_1.map(create_input_ids, remove_columns=["text", "codes_list"])
    columns_to_keep = ["input_ids", "labels", "attention_mask"]
    columns_to_remove = [
        col for col in dataset_1.column_names if col not in columns_to_keep
    ]
    dataset_1 = dataset_1.remove_columns(
        columns_to_remove
    )  # Determine whether to include the source field
    return dataset_1, snac_model
...
def _(
    ...
    all_padded_tensors = []
    all_attention_masks = []  # Start of human
    max_length = max(
    ...
    del my_samples, samples  # codes = [c.to("cuda") for c in codes]
    return
...
def _(model_1, tokenizer):
    model_1.save_pretrained("orpheus_lora")
...
def _(model_1, tokenizer):
    ...
    if False:
        model_1.save_pretrained_merged(
            "orpheus_finetune_16bit", tokenizer, save_method="merged_16bit"
        )
    if False:
        # Merge to 4bit
        model_1.push_to_hub_merged(
            "HF_USERNAME/orpheus_finetune_16bit",
            tokenizer,
    ...
    if False:
        model_1.save_pretrained_merged(
            "orpheus_finetune_4bit", tokenizer, save_method="merged_4bit"
        )
    # Just LoRA adapters
    if False:
        model_1.push_to_hub_merged(
            "HF_USERNAME/orpheus_finetune_4bit",
            tokenizer,
    ...
    if False:
        ...
        tokenizer.save_pretrained("orpheus_lora")
    if False:
        model_1.push_to_hub("HF_USERNAME/orpheus_lora", token="YOUR_HF_TOKEN")
