...
def _():
    ...
    model, tokenizer = FastLanguageModel.from_pretrained(
        model_name="unsloth/granite-4.0-350m-unsloth-bnb-4bit",  # YOUR MODEL YOU USED FOR TRAINING
...
def _():
    ...
    dataset = load_dataset(
        ...
        column_names=[  # Replace with the actual column names of your sheet
            "snippet",
            "recommendation",
...
def _(model_1, tokenizer):
    model_1.save_pretrained("granite_lora")
...
def _():
    if False:
        ...
        _model, _tokenizer = _FastLanguageModel.from_pretrained(
            model_name="granite_lora", max_seq_length=2048, load_in_4bit=True  # YOUR MODEL YOU USED FOR TRAINING
        )
    return
...
def _(model_1, tokenizer):
    ...
    if False:
        model_1.save_pretrained_merged(
            "granite_finetune_16bit", tokenizer, save_method="merged_16bit"
        )
    ...
    if False:
        # Merge to 4bit
        model_1.save_pretrained_merged(
            "granite_finetune_4bit", tokenizer, save_method="merged_4bit"
        )
    if False:
        model_1.push_to_hub_merged(
    ...
    if False:
        ...
        tokenizer.save_pretrained("granite_lora")
    if False:
        model_1.push_to_hub("HF_USERNAME/granite_lora", token="YOUR_HF_TOKEN")
        tokenizer.push_to_hub(
            "HF_USERNAME/granite_lora", token="YOUR_HF_TOKEN"
        )  # Pushing to HF Hub
    return
...
def _(model_1, tokenizer):
    # Save to 8bit Q8_0
    if False:
        model_1.save_pretrained_gguf("granite_finetune", tokenizer)
    if False:
    ...
    if False:
        ...
        model_1.save_pretrained_gguf(
            "granite_finetune", tokenizer, quantization_method="f16"
        )
    if False:
        model_1.push_to_hub_gguf(
