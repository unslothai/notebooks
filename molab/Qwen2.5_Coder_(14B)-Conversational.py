...
def _():
    ...
    model, tokenizer = FastLanguageModel.from_pretrained(
        model_name="unsloth/Qwen2.5-Coder-14B-Instruct",  # YOUR MODEL YOU USED FOR TRAINING
...
def _(tokenizer):
    ...
    tokenizer_1 = get_chat_template(tokenizer, chat_template="qwen-2.5")
...
def _(FastLanguageModel, get_chat_template, model_1, tokenizer_1):
    tokenizer_2 = get_chat_template(tokenizer_1, chat_template="qwen-2.5")
...
def _(model_1, tokenizer_2):
    ...
    tokenizer_2.save_pretrained("qwen_lora")
    return
...
def _(load_in_4bit):
    if False:
        ...
        _model = _AutoPeftModelForCausalLM.from_pretrained(
            "qwen_lora", load_in_4bit=load_in_4bit
        )
        _tokenizer = _AutoTokenizer.from_pretrained("qwen_lora")
    return
...
def _(model_1, tokenizer_2):
    ...
    if False:
        model_1.save_pretrained_merged(
            "qwen_finetune_16bit", tokenizer_2, save_method="merged_16bit"
        )
    if False:
        # Merge to 4bit
    ...
    if False:
        model_1.save_pretrained_merged(
            "qwen_finetune_4bit", tokenizer_2, save_method="merged_4bit"
        )
    # Just LoRA adapters
    if False:
        model_1.push_to_hub_merged(
    ...
    if False:
        ...
        tokenizer_2.push_to_hub("HF_USERNAME/qwen_lora", token="YOUR_HF_TOKEN")
    return
...
def _(model_1, tokenizer_2):
    ...
    if False:
        ...
        model_1.save_pretrained_gguf("qwen_finetune", tokenizer_2)
    if False:
        # Save to 16bit GGUF
        model_1.push_to_hub_gguf(
            "HF_USERNAME/qwen_finetune", tokenizer_2, token="YOUR_HF_TOKEN"
        )
    if False:
        model_1.save_pretrained_gguf(
            "qwen_finetune", tokenizer_2, quantization_method="f16"
        )
    # Save to q4_k_m GGUF
    if False:
        model_1.push_to_hub_gguf(
    ...
    if False:
        ...
        model_1.save_pretrained_gguf(
            "qwen_finetune", tokenizer_2, quantization_method="q4_k_m"
        )
    if False:
        model_1.push_to_hub_gguf(
            "HF_USERNAME/qwen_finetune",
