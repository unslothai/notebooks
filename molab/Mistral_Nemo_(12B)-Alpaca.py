...
def _():
    ...
    model, tokenizer = FastLanguageModel.from_pretrained(
        model_name="unsloth/Mistral-Nemo-Base-2407",  # YOUR MODEL YOU USED FOR TRAINING
...
def _(mo):
    ...
    **[NOTE]** Remember to add the **EOS_TOKEN** to the tokenized output! Otherwise you'll get infinite generations!
    ...
    If you want to use the `llama-3` or `mistral` template for ShareGPT datasets, try our conversational [notebook](https://github.com/unslothai/notebooks/blob/main/nb/Llama3_(8B)-Conversational.ipynb).
...
def _(tokenizer):
    ...
    def formatting_prompts_func(examples):
        ...
        texts = []
        for instruction, input, output in zip(instructions, _inputs, outputs):
            text = alpaca_prompt.format(instruction, input, output) + EOS_TOKEN
            texts.append(text)
        return {"text": texts}  # Must add EOS_TOKEN
...
def _(model_1, tokenizer):
    model_1.save_pretrained("mistral_nemo_lora")
...
def _(load_in_4bit):
    if False:
        ...
        _model = _AutoPeftModelForCausalLM.from_pretrained(
            "mistral_nemo_lora", load_in_4bit=load_in_4bit
        )
        _tokenizer = _AutoTokenizer.from_pretrained("mistral_nemo_lora")
    return
...
def _(model_1, tokenizer):
    ...
    if False:
        model_1.save_pretrained_merged(
            "mistral_nemo_finetune_16bit", tokenizer, save_method="merged_16bit"
        )
    if False:
        # Merge to 4bit
        model_1.push_to_hub_merged(
            "HF_USERNAME/mistral_nemo_finetune_16bit",
            tokenizer,
    ...
    if False:
        model_1.save_pretrained_merged(
            "mistral_nemo_finetune_4bit", tokenizer, save_method="merged_4bit"
        )
    # Just LoRA adapters
    if False:
        model_1.push_to_hub_merged(
            "HF_USERNAME/mistral_nemo_finetune_4bit",
            tokenizer,
    ...
    if False:
        ...
        tokenizer.save_pretrained("mistral_nemo_lora")
    if False:
        model_1.push_to_hub("HF_USERNAME/mistral_nemo_lora", token="YOUR_HF_TOKEN")
...
def _(model_1, tokenizer):
    ...
    if False:
        ...
        model_1.save_pretrained_gguf("mistral_nemo_finetune", tokenizer)
    if False:
        # Save to 16bit GGUF
        model_1.push_to_hub_gguf(
            "HF_USERNAME/mistral_nemo_finetune", tokenizer, token="YOUR_HF_TOKEN"
        )
    if False:
        model_1.save_pretrained_gguf(
            "mistral_nemo_finetune", tokenizer, quantization_method="f16"
        )
    # Save to q4_k_m GGUF
    if False:
        model_1.push_to_hub_gguf(
            "HF_USERNAME/mistral_nemo_finetune",
            tokenizer,
    ...
    if False:
        ...
        model_1.save_pretrained_gguf(
            "mistral_nemo_finetune", tokenizer, quantization_method="q4_k_m"
        )
    if False:
        model_1.push_to_hub_gguf(
            "HF_USERNAME/mistral_nemo_finetune",
