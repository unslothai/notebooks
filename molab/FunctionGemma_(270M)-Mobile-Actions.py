...
def _(mo):
    mo.md(r"""
    <a href="https://github.com/unslothai/notebooks/blob/main/nb/FunctionGemma_(270M)-Mobile-Actions.ipynb" target="_parent"><img src="https://marimo.io/molab-shield.svg" alt="Open In Colab"/></a>
    """)
    return
...
def _():
    ...
    model, tokenizer = FastLanguageModel.from_pretrained(
        model_name="unsloth/functiongemma-270m-it",  # YOUR MODEL YOU USED FOR TRAINING
...
def _(dataset, tokenizer):
    ...
    dataset_1 = dataset.map(process_dataset, fn_kwargs={"tokenizer": tokenizer})
    return (dataset_1,)
...
def _(dataset_1, model_1, tokenizer):
    ...
    split_dataset = dataset_1.train_test_split(test_size=50, shuffle=True, seed=3407)
...
def _(model_1, tokenizer):
    model_1.save_pretrained("functiongemma_lora")
...
def _():
    if False:
        ...
        _model, _tokenizer = _FastLanguageModel.from_pretrained(
            model_name="functiongemma_lora", max_seq_length=2048, load_in_4bit=False  # YOUR MODEL YOU USED FOR TRAINING
        )
    return
...
def _(model_1, tokenizer):
    ...
    if False:
        model_1.save_pretrained_merged(
            "functiongemma_finetune_16bit", tokenizer, save_method="merged_16bit"
        )
    if False:  # Pushing to HF Hub
        model_1.push_to_hub_merged(
            "HF_USERNAME/functiongemma_finetune_16bit",
            tokenizer,
    ...
    if False:
        # Merge to 4bit
        model_1.save_pretrained_merged(
            "functiongemma_finetune_4bit", tokenizer, save_method="merged_4bit"
        )
    if False:
        model_1.push_to_hub_merged(
            "HF_USERNAME/functiongemma_finetune_4bit",
            tokenizer,
    ...
    if False:
        ...
        tokenizer.save_pretrained("functiongemma_lora")
    if False:
        model_1.push_to_hub("HF_USERNAME/functiongemma_lora", token="YOUR_HF_TOKEN")
        tokenizer.push_to_hub(
            "HF_USERNAME/functiongemma_lora", token="YOUR_HF_TOKEN"
        )  # Pushing to HF Hub
    return
...
def _(model_1, tokenizer):
    if False:  # Change to True to save to GGUF
        model_1.save_pretrained_gguf(
            "functiongemma_finetune", tokenizer, quantization_method="Q8_0"
