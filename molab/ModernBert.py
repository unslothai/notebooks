...
@app.cell
def _(model_1):
    model_1.save_pretrained("modernbert_lora")
    model_1.tokenizer.save_pretrained("modernbert_lora")
    return
...
def _(model_1):
    ...
    if False:
        model_1.save_pretrained_merged(
            "modernbert_finetune_16bit",
            tokenizer=model_1.tokenizer,
            save_method="merged_16bit",
        )
    if False:  # Pushing to HF Hub
        model_1.push_to_hub_merged(
            "HF_USERNAME/modernbert_finetune_16bit",
            tokenizer=model_1.tokenizer,
            save_method="merged_16bit",
    ...
    if False:
        model_1.push_to_hub(
            "HF_USERNAME/modernbert_lora", token="YOUR_HF_TOKEN"
        )  # Pushing to HF Hub
    return
...
def _(model_1):
    # Save to 8bit Q8_0
    if False:
        model_1.save_pretrained_gguf("modernbert_finetune")
    if False:
        model_1.push_to_hub_gguf(
            "HF_USERNAME/modernbert_finetune", token="YOUR_HF_TOKEN"
        )
    if False:
        # Save to 16bit GGUF
        model_1.save_pretrained_gguf("modernbert_finetune", quantization_method="f16")
    if False:
        model_1.push_to_hub_gguf(
            "HF_USERNAME/modernbert_finetune",
            quantization_method="f16",
            token="YOUR_HF_TOKEN",  # Get a token at https://huggingface.co/settings/tokens
        )  # Pushing to HF Hub
    if False:
        model_1.save_pretrained_gguf(
            "modernbert_finetune", quantization_method="q4_k_m"
        )
    # Save to q4_k_m GGUF
    if False:
        model_1.push_to_hub_gguf(
