...
def _(mo):
    ...
    In this [Hugging Face](https://huggingface.co/learn/nlp-course/en/chapter12/6?fw=pt) and Unsloth notebook, you will learn to transform Qwen3 8B FP8 GRPO into a Reasoning model using GRPO.
...
def _(model, tokenizer):
    ...
    _ = model.generate(
        **inputs, max_new_tokens=512, use_cache=True, streamer=TextStreamer(tokenizer)
    )
    return (TextStreamer,)
...
@app.cell
def _(TextStreamer, dataset_2, model, tokenizer):
...
def _(SamplingParams, model):
    text_2 = "What is the sqrt of 101?"
    sampling_params_1 = SamplingParams(temperature=1.0, top_k=50, max_tokens=256)
    output_1 = (
        model.fast_generate(
            [text_2], sampling_params=sampling_params_1, lora_request=None
        )[0]
        .outputs[0]
        .text
    )
...
@app.cell
def _(SamplingParams, max_seq_length, maximum_length, tokenizer):
    ...
    training_args = GRPOConfig(
        ...
        temperature=1.0,
        learning_rate=5e-06,  # Reduce to 2e-5 for long training runs
        weight_decay=0.01,
        warmup_ratio=0.1,
        lr_scheduler_type="linear",
        optim="adamw_8bit",
...
@app.cell
def _(SamplingParams, model):
    text_3 = "What is the sqrt of 101?"
    sampling_params_2 = SamplingParams(temperature=1.0, top_k=50, max_tokens=1024)
    output_2 = (
        model.fast_generate(
            [text_3], sampling_params=sampling_params_2, lora_request=None
        )[0]
        .outputs[0]
        .text
    )
