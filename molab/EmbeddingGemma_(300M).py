...
def _():
    ...
    stream_train = list(
        load_dataset("tomaarsen/miriad-4.4M-split", split="train", streaming=True).take(
            10000
        )
    )
    stream_eval = list(
        load_dataset("tomaarsen/miriad-4.4M-split", split="eval", streaming=True).take(
            2000
        )
    )
...
@app.cell
def _(eval_dataset, model_1, train_dataset):
    import torch
    ...
    evaluator = InformationRetrievalEvaluator(  # Optional, will make training slower
        queries=queries,
        corpus=corpus,
    ...
    with torch.autocast(
        device_type="cuda", dtype=model_1.dtype, enabled=model_1.dtype != torch.float16
    ):
        print(evaluator(model_1))
    return evaluator, torch
...
@app.cell
def _(eval_dataset, evaluator, model_1, train_dataset):
    ...
    trainer = SentenceTransformerTrainer(
        ...
        args=SentenceTransformerTrainingArguments(
            ...
            prompts={  # Map training column names to model prompts
                "question": model_1.prompts["query"],
                "passage_text": model_1.prompts["document"],
            },
...
def _(model_1):
    ...
    query_emb = model_1.encode(query, convert_to_tensor=True)
    candidate_embs = model_1.encode(candidates, convert_to_tensor=True)
    similarities = model_1.similarity(query_emb, candidate_embs)
...
def _():
    if False:
        ...
        _model = _FastSentenceTransformer.from_pretrained("model")
    return
...
def _(model_1):
    ...
    if False:
        model_1.push_to_hub(
            "HF_USERNAME/embeddinggemma_lora", token="YOUR_HF_TOKEN"
        )  # Pushing to HF Hub
    return
...
def _(model_1):
    # Save to 8bit Q8_0
    if False:
        model_1.save_pretrained_gguf("embeddinggemma_finetune")
    if False:
        model_1.push_to_hub_gguf(
            "HF_USERNAME/embeddinggemma_finetune", token="YOUR_HF_TOKEN"
        )
    if False:
        # Save to 16bit GGUF
        model_1.save_pretrained_gguf(
            "embeddinggemma_finetune", quantization_method="f16"
        )
    if False:
        model_1.push_to_hub_gguf(
            "HF_USERNAME/embeddinggemma_finetune",
            quantization_method="f16",
            token="YOUR_HF_TOKEN",  # Get a token at https://huggingface.co/settings/tokens
        )  # Pushing to HF Hub
    if False:
        model_1.save_pretrained_gguf(
            "embeddinggemma_finetune", quantization_method="q4_k_m"
        )
    # Save to q4_k_m GGUF
    if False:
        model_1.push_to_hub_gguf(
