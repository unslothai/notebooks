...
def _(mo):
    mo.md(r"""
    <a href="https://github.com/unslothai/notebooks/blob/main/nb/Advanced_Llama3_2_(3B)_GRPO_LoRA.ipynb" target="_parent"><img src="https://marimo.io/molab-shield.svg" alt="Open In Colab"/></a>
    """)
    return
...
def _(reasoning_end, reasoning_start, solution_end, solution_start):
    def match_format_approximately(completions, **kwargs):
        ...
        for completion in completions:
            ...
            score = score + (0.5 if response.count(solution_start) == 1 else -1.0)
...
def _(model, tokenizer):
    ...
    if False:
        model.push_to_hub_merged(
            "HF_USERNAME/advanced_finetune_16bit",
            tokenizer,
            save_method="merged_16bit",
            token="YOUR_HF_TOKEN",
        )
    ...
    if False:
        model.save_pretrained_merged(
            "advanced_finetune_4bit",
            tokenizer,
            save_method="merged_4bit",
        )
    if False:
        model.push_to_hub_merged(
            "HF_USERNAME/advanced_finetune_4bit",
            tokenizer,
            save_method="merged_4bit",
            token="YOUR_HF_TOKEN",
        )
...
def _(model, tokenizer):
    ...
    if False:
        model.push_to_hub_gguf(
            "HF_USERNAME/advanced_finetune", tokenizer, token="YOUR_HF_TOKEN"
        )

    # Save to 16bit GGUF
    if False:
        model.save_pretrained_gguf(
            "advanced_finetune", tokenizer, quantization_method="f16"
        )
    if False:
        model.push_to_hub_gguf(
            "HF_USERNAME/advanced_finetune",
            tokenizer,
    ...
    if False:
        model.save_pretrained_gguf(
            "advanced_finetune", tokenizer, quantization_method="q4_k_m"
        )
    if False:
        model.push_to_hub_gguf(
            "HF_USERNAME/advanced_finetune",
