...
def _():
    ...
    model, tokenizer = FastLanguageModel.from_pretrained(
        model_name="unsloth/GLM-4.7-Flash",  # YOUR MODEL YOU USED FOR TRAINING
...
@app.cell
def _(model_1, tokenizer):
    model_1.save_pretrained("glm_flash_lora")
...
def _():
    if False:
        ...
        _model, _tokenizer = _FastLanguageModel.from_pretrained(
            model_name="glm_flash_lora", max_seq_length=2048, load_in_4bit=True  # YOUR MODEL YOU USED FOR TRAINING
        )
    return
...
def _(model_1, tokenizer):
    ...
    if False:
        model_1.save_pretrained_merged(
            "glm_flash_finetune_16bit", tokenizer, save_method="merged_16bit"
        )
    if False:  # Pushing to HF Hub
        model_1.push_to_hub_merged(
            "HF_USERNAME/glm_flash_finetune_16bit",
            tokenizer,
    ...
    if False:
        # Merge to 4bit
        model_1.save_pretrained_merged(
            "glm_flash_finetune_4bit", tokenizer, save_method="merged_4bit"
        )
    if False:
        model_1.push_to_hub_merged(
            "HF_USERNAME/glm_flash_finetune_4bit",
            tokenizer,
    ...
    if False:
        ...
        tokenizer.save_pretrained("glm_flash_lora")
    if False:
        model_1.push_to_hub("HF_USERNAME/glm_flash_lora", token="YOUR_HF_TOKEN")
        tokenizer.push_to_hub(
            "HF_USERNAME/glm_flash_lora", token="YOUR_HF_TOKEN"
        )  # Pushing to HF Hub
    return
...
def _(model_1, tokenizer):
    # Save to 8bit Q8_0
    if False:
        model_1.save_pretrained_gguf("glm_flash_finetune", tokenizer)
    if False:
        model_1.push_to_hub_gguf(
            "HF_USERNAME/glm_flash_finetune", tokenizer, token="YOUR_HF_TOKEN"
        )
    if False:
        # Save to 16bit GGUF
        model_1.save_pretrained_gguf(
            "glm_flash_finetune", tokenizer, quantization_method="f16"
        )
    if False:
        model_1.push_to_hub_gguf(
            "HF_USERNAME/glm_flash_finetune",
            tokenizer,
    ...
    if False:
        model_1.save_pretrained_gguf(
            "glm_flash_finetune", tokenizer, quantization_method="q4_k_m"
        )
    # Save to q4_k_m GGUF
    if False:
        model_1.push_to_hub_gguf(
